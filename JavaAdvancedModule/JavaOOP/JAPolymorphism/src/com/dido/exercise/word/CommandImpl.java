...
public class CommandImpl implements CommandInterface {
    ...
    class CutTransform implements TextTransform {
        ...
        private String lastRemovedElement;

        @Override
        public void invokeOn(StringBuilder text, int startIndex, int endIndex) {
            if (startIndex == endIndex) {
                setLastRemovedElement("");
                return;
            }

            setLastRemovedElement(text.substring(startIndex, endIndex));
            text.replace(startIndex,endIndex,"");
        }

        public String getLastRemovedElement() {
            return lastRemovedElement;
        }

        private void setLastRemovedElement(String lastRemovedElement) {
            this.lastRemovedElement = lastRemovedElement;
        }
    }
    ...
    class PasteTransform implements TextTransform {
        ...
        @Override
        public void invokeOn(StringBuilder text, int startIndex, int endIndex) {
            String lastRemovedElement = ((CutTransform)commandTransforms.get("cut"))
                    .getLastRemovedElement();
            String originalText = text.toString();
            text.delete(0, text.length());
            text.append(originalText.substring(0, startIndex));
            text.append(lastRemovedElement);
            text.append(originalText.substring(endIndex));
        }
    }
